...
#include "GAS/ExecutionCalculations/HIDamage3Calculation.h"


UHIDamage3Calculation::UHIDamage3Calculation(const FObjectInitializer& objectInitializer) : Super(objectInitializer)
{
	RelevantAttributesToCapture.Add(structAttributes.damage3SourceDef);
...
void UHIDamage3Calculation::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, OUT FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	if (werewolfAbilitySystem && tragetAbilitySystem)
	{
		...
		float damage = 0.f;
		ensure(ExecutionParams.AttemptCalculateCapturedAttributeMagnitude(structAttributes.damage3SourceDef, evaluationParameters, damage));
		...
		{
			FGameplayModifierEvaluatedData enemyHeathModification = FGameplayModifierEvaluatedData(structAttributes.healthTargetProperty, EGameplayModOp::Additive, -damage);
			OutExecutionOutput.AddOutputModifier(enemyHeathModification);
		}
	}
}
