...
#include "GAS/ExecutionCalculations/HIDamage2Calculation.h"


UHIDamage2Calculation::UHIDamage2Calculation(const FObjectInitializer& objectInitializer) : Super(objectInitializer)
{
    RelevantAttributesToCapture.Add(structAttributes.damage2SourceDef);
...
void UHIDamage2Calculation::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, OUT FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
    ...
    if (werewolfAbilitySystem && tragetAbilitySystem)
    {
        ...
        float damage = 0.f;
        ensure(ExecutionParams.AttemptCalculateCapturedAttributeMagnitude(structAttributes.damage2SourceDef, evaluationParameters, damage));
